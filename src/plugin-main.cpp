...
static std::vector<std::string> get_audio_sources() {
    ...
    obs_enum_sources([](void *p,obs_source_t *s)->bool{
        ...
        if(obs_source_get_output_flags(s)&OBS_SOURCE_AUDIO) v->push_back(obs_source_get_name(s));
        return true;},&r);
    return r;
}
static std::vector<std::string> get_scene_names() {
    ...
    if(n){for(char **p=n;*p;++p) r.push_back(*p); bfree(n);}
    return r;
}
static void apply_config() {
    ...
    g_monitor->clear();
    for(auto &m:g_config->get_mappings()) g_monitor->add_source(m.audio_source);
    if(g_dock){
        std::vector<std::pair<std::string,std::string>> pairs;
        for(auto &m:g_config->get_mappings()) pairs.emplace_back(m.audio_source,m.scene_name);
...
static void do_switch(const std::string &scene_name) {
    obs_queue_task(OBS_TASK_UI,[](void *param){
        ...
        if(g_config->get_transition_fade()){
            ...
            if(ft){obs_frontend_set_current_transition(ft);obs_frontend_set_transition_duration(g_config->get_fade_duration_ms());obs_source_release(ft);}
        }
...
static void on_frontend_event(enum obs_frontend_event event,void*) {
    if(event==OBS_FRONTEND_EVENT_FINISHED_LOADING||event==OBS_FRONTEND_EVENT_SCENE_LIST_CHANGED){
        apply_config();
        if(g_settings_dlg) g_settings_dlg->populate_sources(get_audio_sources(),get_scene_names());
    }
}
bool obs_module_load() {
    blog(LOG_INFO,"[switchy] Loading Switchy v1.0.0");
    g_config=new Config(); g_engine=new SwitchEngine(); g_monitor=new AudioMonitor();
    g_monitor->set_callback([](const std::string &s,float d){g_engine->on_audio_level(s,d);});
    ...
    QObject::connect(g_settings_dlg,&SettingsDialog::settings_applied,[](const Config&){apply_config();});
    QObject::connect(g_dock,&AutoCamDock::open_settings_requested,[](){
        ...
        g_settings_dlg->show(); g_settings_dlg->raise(); g_settings_dlg->activateWindow();
    });
...
void obs_module_unload() {
    ...
    g_engine->set_enabled(false); g_monitor->clear();
    delete g_settings_dlg; delete g_monitor; delete g_engine; delete g_config;
    g_settings_dlg=nullptr; g_monitor=nullptr; g_engine=nullptr; g_config=nullptr;
}
const char *obs_module_name() { return "Switchy"; }
const char *obs_module_description() { return "Podcast auto camera switcher inspired by RorodeCaster Video."; }
